...
class Evaluator:
    ...
    def evaluate_propagation(self, test_size: float = 0.3, k: int = 5) -> Dict[str, float]:
        """
        Evaluate label propagation on the full labeled dataset (train and predict on all methods).
        ...
        print(f"📊 Evaluating on full dataset: {len(methods)} methods")
        
        # Train k-NN classifier on all data
        ...
        print("-" * 50)
        conflicts = []
        for actual, predicted, method in zip(y, y_pred, methods):
            if actual != predicted:
                conflicts.append({
                    'method': method,
                    'actual': actual,
        ...
        plot_confusion_matrix(y, y_pred, 'Label Propagation - Confusion Matrix')
